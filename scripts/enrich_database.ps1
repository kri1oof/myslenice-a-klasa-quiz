...
Write-Host "[1/16] Katalog historycznych klubow"
Run-Quiz import-club-catalog data/historical_clubs_seed.csv

Write-Host "[2/16] Historia uczestnikow - bez automatycznego KtoWygral"
Write-Host "KtoWygral blokuje seryjne zapytania (HTTP 429). Pomijam ten serwis, aby nie obchodzic limitow."

Write-Host "[3/16] Tabele, terminarze, kolejki i daty z RegionalnyFutbol (2017/18-2025/26)"
Run-Quiz sync-regionalny-history --start-year 2017 --end-year 2025

Write-Host "[4/16] Zweryfikowany kompletny sezon 2019/20"
Run-Python scripts/import_2019_20_reference.py

Write-Host "[5/16] Biezacy sezon 2026/27 z oficjalnego MZPN"
...
Write-Host "[6/16] Historyczne terminarze Futbolowo 2014/15-2018/19 i 2021/22"
$futbolowoSchedules = @(
...
if ($DeepFutbolowo) {
  Write-Host "[7/16] Szczegoly dostepnych meczow Futbolowo"
  foreach ($archive in $futbolowoSchedules) {
    Run-Quiz crawl-futbolowo-schedule --season $archive.Season --delay 0.5 $archive.Url
  }
} else {
  Write-Host "[7/16] Pomijam gleboki crawl Futbolowo. Uzyj -DeepFutbolowo, aby pobrac szczegoly meczow."
}

Write-Host "[8/16] Archiwa SportoweTempo 2009/10-2012/13: tabele, terminarze i linki relacji"
$sportoweTempo = @(
...
if ($DeepSportoweTempo) {
  Write-Host "[9/16] Gleboki crawl relacji SportoweTempo"
  foreach ($archive in $sportoweTempo) {
    Run-Quiz crawl-sportowetempo-season --season $archive.Season --delay 0.5 $archive.Url
  }
} else {
  Write-Host "[9/16] Pomijam gleboki crawl SportoweTempo. Uzyj -DeepSportoweTempo, aby pobrac gole i sklady z relacji."
}

Write-Host "[10/16] Naprawa identyfikacji historycznych meczow po kolejce"
Run-Python scripts/repair_sportowetempo_round_matches.py

Write-Host "[11/16] Zweryfikowane korekty wynikow"
Run-Python scripts/apply_verified_match_corrections.py

Write-Host "[12/16] Zweryfikowane fakty meczowe i kontekst lokalny"
Run-Python scripts/import_social_context.py

Write-Host "[13/16] Herby klubow"
Run-Quiz fetch-crests --delay 0.25

Write-Host "[14/16] Audyt danych"
Run-Quiz audit-data

Write-Host "[15/16] Generator pytan"
Run-Quiz generate

Write-Host "[16/16] Eksport gry"
Run-Quiz export
